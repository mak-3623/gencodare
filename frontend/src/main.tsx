...
function GraphView({ graph, statuses, path, onReset, onQuiz }: { graph: Graph; statuses: Record<string, Status>; path: PathStep[]; onReset: () => void; onQuiz: () => void }) {
  const { nodes: baseNodes, edges } = useMemo(() => layout(graph), [graph]); const [selected, setSelected] = useState<{ title: string; body: string; status: Status; sources: string[] } | null>(null), [view, setView] = useState<'graph' | 'path'>('graph')
  ...
  const onNodeClick: NodeMouseHandler = useCallback((_, node) => setSelected({ title: String(node.data.label), body: String(node.data.description), status: (node.data.status ?? 'unknown') as Status, sources: Array.isArray(node.data.sources) ? node.data.sources.map(String) : [] }), [])
  const onEdgeClick: EdgeMouseHandler = useCallback((_, edge) => setSelected({ title: 'Why this connection?', body: String(edge.data?.reason ?? ''), status: 'unknown', sources: [] }), [])
