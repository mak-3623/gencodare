...
type Concept = { id: string; name: string; short_description: string; sources: string[] }
...
const layout = (graph: Graph) => {
  ...
  dagre.layout(dag)
  return {
    nodes: graph.nodes.map(node => { const p = dag.node(node.id); return { id: node.id, data: { label: node.name, description: node.short_description, sources: node.sources }, position: { x: p.x - 105, y: p.y - 35 }, className: 'concept-node' } as Node }),
...
function GraphView({ graph, statuses, path, onReset, onQuiz }: { graph: Graph; statuses: Record<string, Status>; path: PathStep[]; onReset: () => void; onQuiz: () => void }) {
  const { nodes: baseNodes, edges } = useMemo(() => layout(graph), [graph]); const [selected, setSelected] = useState<{ title: string; body: string; status: Status; sources: string[]; learningReason: string } | null>(null), [view, setView] = useState<'graph' | 'path'>('graph')
  ...
  const onNodeClick: NodeMouseHandler = useCallback((_, node) => { const dependentNames = graph.edges.filter(edge => edge.from === node.id).map(edge => graph.nodes.find(concept => concept.id === edge.to)?.name).filter(Boolean); const prerequisiteNames = graph.edges.filter(edge => edge.to === node.id).map(edge => graph.nodes.find(concept => concept.id === edge.from)?.name).filter(Boolean); const learningReason = dependentNames.length ? `Study this early because it supports ${dependentNames.slice(0, 2).join(' and ')}${dependentNames.length > 2 ? ' and later concepts' : ''}.` : prerequisiteNames.length ? `Study this after ${prerequisiteNames.slice(0, 2).join(' and ')} so its prerequisites are in place.` : 'This is a foundational starting point for the learning path.'; setSelected({ title: String(node.data.label), body: String(node.data.description), status: (node.data.status ?? 'unknown') as Status, sources: Array.isArray(node.data.sources) ? node.data.sources.map(String) : [], learningReason }) }, [graph])
  const onEdgeClick: EdgeMouseHandler = useCallback((_, edge) => setSelected({ title: 'Why this connection?', body: String(edge.data?.reason ?? ''), status: 'unknown', sources: [], learningReason: '' }), [])
